...
function mapSessionToOverview(session: ParsedSession) {
  ...
  let json = JSON.stringify(overview)
  if (json.length > L1_RESPONSE_LIMIT) {
    // Collect all message fields with their lengths, sorted longest first
    ...
    for (const entry of entries) {
      ...
      if (current && current.length > 200) {
        ...
        if (json.length <= L1_RESPONSE_LIMIT) break
      }
    }
  }
...
function mapTurnToSummary(turn: Turn, turnIndex: number) {
  ...
  // (parser tracks every progress event; the last one has the most complete data)
...
function mapContentBlock(block: TurnContentBlock) {
  switch (block.kind) {
    case "thinking": {
      // Concatenate thinking blocks, skip redacted ones (empty thinking with signature)
...
// Find parent tool call ID that spawned this agent
function findParentToolCallId(session: ParsedSession, agentId: string): string | null {
  for (const turn of session.turns) {
    for (const msg of turn.subAgentActivity) {
      if (msg.agentId === agentId) {
        // Find the Task/Agent tool call that has this agentId
        for (const tc of turn.toolCalls) {
